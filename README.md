...
about a dozen more, and "not stated" is the correct answer for the silent ones, this is
not a gap in the data, but a fact about the document a model can get right or
wrong. A model that reads a temperature-sensor datasheet, correctly reports its specs, and then
fills in a plausible field-of-view that appears nowhere in the text scores
**precision 1.0** — and is the model you should be careful with, because a fabricated
...
temperature, pressure, magnetometer. All are **simple single-supply parts** where
...
- **Single-label ground truth**, one human verification pass. Corrections are welcome: 'corpus/sensors/gold.json'.
